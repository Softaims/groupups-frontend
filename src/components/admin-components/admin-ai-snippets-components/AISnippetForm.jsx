      ...
      <div className="bg-[#1a1e24] rounded-lg border border-[#2a2e34] w-full max-w-2xl">
        <div className="flex items-center justify-between p-4 border-b border-[#2a2e34]">
          <h2 className="text-xl font-semibold text-white">
            {isEditMode ? "Edit AI Snippet" : "Add AI Snippet"}
          </h2>
          <button
            onClick={onClose}
            className="p-2 text-gray-400 cursor-pointer hover:text-white hover:bg-[#0c0f12] rounded-md transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={onSubmit} className="p-4 space-y-4">
          <div>
            ...
            />
            {errors.snippet_text && (
              <p className="mt-1 text-sm text-red-500">{errors.snippet_text}</p>
            )}
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#2a2e34]">
            <button
              ...
              onClick={onClose}
              disabled={isLoading}
              className="px-4 py-2 cursor-pointer text-gray-400 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isLoading}
              className="px-4 py-2 cursor-pointer bg-[#3CBFAE] text-white rounded-md hover:bg-[#35ab9c] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading
                ? "Saving..."
                : isEditMode
